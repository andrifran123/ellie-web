        ...
        <div style={{ display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr" }}>
          <Field label="API Base" hint="Render URL (e.g. https://ellie-api-1.onrender.com)">
            <input value={apiBase} onChange={(e)=>setApiBase(e.target.value)}
              placeholder="https://..." style={{ width: "100%", padding: 8, borderRadius: 12 }} />
          </Field>
        ...
        <div style={{ marginTop: 16, background: "#111", borderRadius: 16, padding: 16 }}>
          <div style={{ display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr" }}>
            <Field label="Base Voice">
              <select value={voice} onChange={(e)=>setVoice(e.target.value)} style={{ width: "100%", padding: 8, borderRadius: 12 }}>
          ...
          <Field label={`Pitch (semitones): ${pitchSemi}`} hint="-12 deeper, +12 brighter">
            <input type="range" min={-12} max={12} step={1} value={pitchSemi} onChange={e=>setPitchSemi(parseFloat(e.target.value))} style={{ width: "100%" }} />
          </Field>
          <Field label={`Tempo: ${tempo.toFixed(2)}`} hint="0.5–2.0 (pitch preserved)">
            <input type="range" min={0.5} max={2} step={0.01} value={tempo} onChange={e=>setTempo(parseFloat(e.target.value))} style={{ width: "100%" }} />
          </Field>
          <Field label={`Stability: ${stability.toFixed(2)}`} hint="More compression = steadier loudness">
            <input type="range" min={0} max={1} step={0.01} value={stability} onChange={e=>setStability(parseFloat(e.target.value))} style={{ width: "100%" }} />
          </Field>
          <Field label={`Clarity: ${clarity.toFixed(2)}`} hint="High-shelf / air">
            <input type="range" min={0} max={1} step={0.01} value={clarity} onChange={e=>setClarity(parseFloat(e.target.value))} style={{ width: "100%" }} />
          </Field>
          <Field label={`Style: ${style.toFixed(2)}`} hint="Room/echo vibe">
            <input type="range" min={0} max={1} step={0.01} value={style} onChange={e=>setStyle(parseFloat(e.target.value))} style={{ width: "100%" }} />
          </Field>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <button onClick={preview} disabled={disabled} style={{ padding: "8px 14px", borderRadius: 16, background: "#fff", color: "#000" }}>Preview</button>
            <button onClick={saveToServer} disabled={disabled} style={{ padding: "8px 14px", borderRadius: 16, border: "1px solid #444" }}>Save Settings</button>
            <button onClick={loadFromServer} disabled={disabled} style={{ padding: "8px 14px", borderRadius: 16, border: "1px solid #444" }}>Load Current</button>
          ...
          <div style={{ marginTop: 16, paddingTop: 12, borderTop: "1px solid #222" }}>
            <div style={{ marginBottom: 8, fontWeight: 600, fontSize: 14 }}>Quick Presets</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {PRESETS.map(p => (
                <button key={p.key} onClick={()=>applyPreset(p.key)} disabled={disabled}
                  style={{ padding: "8px 12px", borderRadius: 16, background: "#1b1b1b", border: "1px solid #333", color: "#ddd" }}>
                  {p.label}
                </button>
              ))}
            </div>
